...
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ExceptionDto> expenseIdNotFound(ExpenseIdNotFoundException ex) {
        ...
        return new ResponseEntity<>(new ExceptionDto("Not Found", ex.getMessage()), HttpStatus.NOT_FOUND);
    }
    ...
    public ResponseEntity<ExceptionDto> currencyCodeNotSupported(CurrencyCodeNotSupportedException ex) {
        logger.error(ex.getMessage());
        return new ResponseEntity<>(new ExceptionDto("Bad Request", ex.getMessage()), HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<ExceptionDto> handleInternal(final RuntimeException ex, final WebRequest request) {
        logger.error(ex.getMessage());
        return new ResponseEntity<>(new ExceptionDto("Internal Server Error", ex.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
    }

    @Override
    protected ResponseEntity<Object> handleHttpMessageNotReadable(final HttpMessageNotReadableException ex, final HttpHeaders headers, final HttpStatus status, final WebRequest request) {
        logger.error(ex.getMessage());
        return handleExceptionInternal(ex, new ExceptionDto("Bad Request", "HTTP message not valid."), headers, HttpStatus.BAD_REQUEST, request);
    }
